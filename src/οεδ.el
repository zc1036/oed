...
(defun draw-todo (todo)
    (when (not (todo-disabled-p todo))
        (let ((line-start-pos (line-beginning-position))
            ...
            (put-text-property line-start-pos (point) 'οεδ-todo-prop todo)
            (newline)
...
(defun draw-subtodos (subtodos)
    (let ((indent-string (make-string drawtodo-indent-level ?\s)))
        (dolist (todo subtodos)
...
(defun οεδ-delete-todo ()
    (interactive)
    (let ((todo-at-point (get-char-property (point) 'οεδ-todo-prop)))
        (when todo-at-point
            (plist-put todo-at-point :disabled t))))
        ...
        (if (buffer-modified-p)
            (save-excursion
                (let ((tmp-buf (generate-new-buffer " οεδ"))
                    ...
                    (mapcar 'write-todo todo)
                    (mapcar 'write-todo done)
                    (save-buffer)
                    (set-buffer buf-name)
                    (kill-buffer tmp-buf)
